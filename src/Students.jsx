...
const  Students = () => {
     ...
     useEffect(() => {
          fetch(`${BaseUrl}/api/v1/timetable?department=${claims.department}`).then(res => {return res.json()}).then((data) => {
     ...
     const generateTimetableByDepartment= () => {
          if(department !== ''){
               fetch(`${BaseUrl}/api/v1/timetable?department=${department}`).then(res => {return res.json()}).then((data) => {
     ...
     return (
          <div>
               ...
               <div className='students__department-div'>
                    ...
                    <button onClick={generateTimetableByDepartment}>Generate Timetable</button>
               </div>
               {timetable?.length > 0 && (
                    timetable?.map((element, index) => (
                         ...
                         {element.courses.map((course, index) => (
                              ...
                              course.day !== element.courses[index - 1].day ? (
                                   ...
                                   rowSpan={element.courses.filter((c) => c.day === course.day).length}
                         ...
                         <button onClick={() => {export2Word(`content-${index}`, "Timetable");}}>
                              Export to DOCX
                         </button>
                         </div>
                    ))
